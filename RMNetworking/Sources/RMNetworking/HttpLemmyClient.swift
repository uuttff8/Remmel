...
public final class HttpLemmyClient: HTTPClientProvider {
    public func getAuth(url: URL, completion: @escaping ((Result<Data, Error>) -> Void)) {
    ...
    public func uploadImage(
        ...
        guard let jwt = LoginData.shared.jwtToken else {
            debugPrint("JWT token is not provided")
            return
        }
...
//                debugPrint(data: data, response: (response as? HTTPURLResponse), error: error)
