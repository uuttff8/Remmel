...
class ReplyMentionCellView: UIView {
    ...
    func configure(reply: LemmyModel.ReplyView) {
        ...
        headerView.bind(
            with: .init(
                ...
                published: reply.published.shortTimeAgoSinceNow,
                score: reply.score,
    ...
    func configure(mention: LemmyModel.UserMentionView) {
        ...
        headerView.bind(
            with: .init(
                ...
                published: mention.published.shortTimeAgoSinceNow,
                score: mention.score,
    ...
    private func setupTargets(with reply: LemmyModel.ReplyView) {
        ...
        footerView.downvoteTap = { [weak self] (scoreView, button, voteType) in
            self?.replyDelegate?.voteContent(scoreView: scoreView, voteButton: button, newVote: voteType, comment: reply)
        }
    ...
    private func setupTargets(with mention: LemmyModel.UserMentionView) {
        ...
        footerView.downvoteTap = { [weak self] (scoreView, button, voteType) in
            self?.mentionDelegate?.voteContent(scoreView: scoreView, voteButton: button, newVote: voteType, comment: mention)
        }
