...
open class CommentCell: SwipeTableViewCell {
    ...
    open var rootCommentMargin: CGFloat! = Appearance.rootCommentMargin {
        didSet {
            updateCommentMargin()
        }
    }
    /// Color of the separation between 2 indented comments
    open var commentMarginColor: UIColor! = Appearance.commentMarginColor {
        didSet {
            updateCommentMargin()
        }
    }
    /// Space between 2 indented comments
    open var commentMargin: CGFloat! = Appearance.commentMargin {
        didSet {
            updateCommentMargin()
        }
    }
    ...
    open var indentationIndicatorColor: UIColor! = Appearance.indentationIndicatorColor {
        didSet {
    ...
    open var indentationIndicatorThickness: CGFloat! = CGFloat(Appearance.indentationIndicatorThickness) {
        didSet {
            updateIndentationIndicators()
        }
    }
    ...
    open var level = 0 {
        didSet {
            /*
             solves a warning
             http://stackoverflow.com/questions/26100053/uitableviewcells-contentview-gets-unwanted-height-44-constraint
            */
    ...
    override public init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        selectionStyle = .none
        indentationColor = Appearance.identationColor
        setupView()
    }
        ...
        if self.level > 0 { // No indicators for root comments
            ...
            for i in 1...n {
                ...
                sep.leadingAnchor.constraint(
                    equalTo: contentView.leadingAnchor,
                    constant: CGFloat((self.level - i + 1) * indentationUnit)
                ).isActive = true
