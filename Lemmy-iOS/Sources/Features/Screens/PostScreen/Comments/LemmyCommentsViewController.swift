...
final class FoldableLemmyCommentsViewController: CommentsViewController {
    ...
    override func viewDidLoad() {
        ...
        tableView.registerClass(SimpleCommentCell.self)
    ...
    override func commentsView(
        ...
        let commentCell: SimpleCommentCell = tableView.cell(forRowAt: indexPath)
        let comment = currentlyDisplayed[indexPath.row] as! LemmyComment
        commentCell.level = comment.level
        commentCell.commentContent = comment.commentContent?.content
        commentCell.posterName = comment.commentContent?.postName
        return commentCell
    }
}
